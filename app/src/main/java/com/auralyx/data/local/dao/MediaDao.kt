...
interface MediaDao {
    ...
    @Query("SELECT * FROM media_items ORDER BY last_played DESC LIMIT 20")
    ...
    @Query("UPDATE media_items SET last_played = :timestamp WHERE id = :id")
    suspend fun updateLastPlayed(id: Long, timestamp: Long)

    @Query("SELECT * FROM media_items WHERE album = :album ORDER BY track_number ASC")
